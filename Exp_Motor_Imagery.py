...
def play_movie(win, file):
    mov = visual.MovieStim(win, file, size=(1920, 1080))
    # 添加一点随机性，提高对受试者的反馈
    # 正常的视频播放顺序
    order = np.linspace(0, 3, 90)
    k = 30
    # 在前k帧中添加随机的时间变化
    order[3:k] = np.random.rand(k-3) * 0.09 + order[3:k]
    # 按照新的顺序播放视频
    for frameN in order[:k]:
        mov.seek(frameN, log=False)  # seek to that frame
...
if __name__ == '__main__':
    ...
    # 日志设置
    logger = init_logger('Exp_Motor_Imagery')
    ...
    controller.store(f'./{save_data_dir}/{subject_id:03d}/block_{block_id:03d}_Exp_Motor_Image')
    time.sleep(5) # 等待数据存储完成
    controller.sess_end()
    exp_df = pd.DataFrame({
        'sub_id': [subject_id],
        'block_id': [block_id],
        'exp_type': ['Exp_Motor_Image'],
